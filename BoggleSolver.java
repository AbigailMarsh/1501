...
public class BoggleSolver
{
	...
	private boolean isValid(int row, int col, int direction){
		...
		//check if its going to be the edge of the board
		if(coords.row < 0 || coords.col < 0 || coords.row > 3 || coords.col > 3){
			return false;
		} else {
			return(!Character.isUpperCase(theBoard[coords.row][coords.col]));
		}
		//then check if the letter is already used
	}
